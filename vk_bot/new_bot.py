...
def get_user_info(user_id):
    ...
    data = {'id': user_info[0].get('id'),'age': str(user_info[0].get('bdate')),'photo': user_info[0].get('photo_200'),'town': user_info[0].get('home_town'),'gender': user_info[0].get('sex'),
...
def send_photo(image_url, user_id, message):
    try:
        ...
        photo = vk_api.upload.VkUpload(vk_session).photo_messages(photos=io.BytesIO(response.content))[0]
        attachment = f"photo{photo['owner_id']}_{photo['id']}"
        vk.messages.send(user_id=user_id,attachment=attachment,message=message,random_id=randrange(10 ** 7))
...
def write_msg(user_id, message, keyboard=''):
    if keyboard:
        vk.messages.send(user_id=user_id, message=message, keyboard=keyboard, random_id=randrange(10 ** 7))
    else:
...
def get_vk_users(offset=0, sex=1, count=10):
    ...
    params = {'fields': 'id,first_name,last_name,bdate,photo_200','v': '5.199','count': count,'sex': sex,'offset': offset,'has_photo': 1}
    try:
        ...
        for user in data_response:
            ...
            data_users.append({'id': user.get('id'),'age': user.get('bdate'),'photo': user.get('photo_200'),'first_name': user.get('first_name'),'last_name': user.get('last_name'),
...
def get_full_photo_candidate(owner_id):
    ...
    token_user = os.getenv('USER_TOKEN')
    headers = {'Authorization': f'Bearer {token_user}'}
    params = {'owner_id': owner_id,'album_id': 'profile','extended': 1,'photo_sizes': 1,'count': 3,'v': '5.199'}
    try:
...
def show_candidate(user_id, candidate):
    ...
    bd_candidate = save_candidate(candidate_vk_id,candidate['first_name'],candidate['last_name'],candidate['profile_link'])
    if not bd_candidate:
...
def start_bot():
    ...
    for event in longpoll.listen():
        ...
        user_info = get_or_create_user(vk_data['id'],vk_data['first_name'],vk_data['last_name'],vk_data['age'],vk_data['town'],vk_data['gender'],photo_url=vk_data['photo'])
        photo = vk_data['photo']
        if user_id in user_states:
            if user_states[user_id] == 'waiting_for_gender':
                if request in ['девушки','девушку', 'ж',]:
                    update_user_preferences(user_id,'girl')
                    del user_states[user_id]
                    write_msg( user_id,"Ваша анкета создана!",keyboard_default())
                    send_photo(photo,user_id,f"{vk_data['first_name']} {vk_data['last_name']},{vk_data['age']}, {vk_data['town']}")
                    ...
                    write_msg(user_id,"Пожалуйста, выберите: девушки или мужчины.",keyboard_default())
                continue
        ...
        if user_id in user_candidate:
            ...
            if request == 'лайк':
                ...
                if index < len(candidates):
                    ...
                    bd_candidate = save_candidate(candidate.get('id'),candidate.get('first_name'),candidate.get('last_name'),candidate.get('profile_link'))
                    if bd_candidate:
                        added = add_to_favorites(user_id,bd_candidate.vk_id)
                        if added:
                            write_msg(user_id,"Кандидат добавлен в избранное",keyboard_like())
                        else:
                            write_msg(user_id,"Этот кандидат уже в избранном.",keyboard_like())
                continue
                ...
                write_msg(user_id,"Анкеты закончились. Попробуйте посмотреть позже.",keyboard_default())
                continue
                ...
                continue
        if request == "начать":
            write_msg(user_id,f"{vk_data['first_name']}, выбери нужный пункт",keyboard_default())
            ...
            if user_info.search_gender == 'girl':
                ...
                write_msg(user_id,"Не удалось определить предпочтения.",keyboard_default())
                continue
            ...
            if user_id not in user_candidate:
                write_msg( user_id,"Сначала начните просмотр анкет.",keyboard_default())
                continue
            state = user_candidate[user_id]
            index = state['index']
            candidates = state['candidates']
            if index < len(candidates):
                ...
                if bd_candidate:
                    ...
                    if added:
                        ...
                        write_msg(user_id,"Кандидат уже находится в избранном.",keyboard_like())
            continue
        elif request == 'следующая анкета':
            ...
            if not favorites:
                ...
                write_msg(user_id,f"У вас {len(favorites)} симпатий.",keyboard_default()
                )
